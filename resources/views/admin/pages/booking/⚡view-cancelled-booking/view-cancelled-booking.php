...
new #[Layout('layouts.app-admin')] class extends Component
{
    ...
    {
        Booking::whereIn('id', $this->selectedBookings)->where('status', 'Cancelled')->delete();
    ...
    {
        if ($value) {
            $this->selectedBookings = Booking::where('status', 'Cancelled')
                ->pluck('id')
                ->map(fn($id) => (int) $id)
                ->toArray();
        } else {
    ...
    {
        $allIds = Booking::where('status', 'Cancelled')->pluck('id')->map(fn($id) => (int) $id)->toArray();
