...
new #[Layout('layouts.app-admin')] class extends Component
{
    ...
    {
        Booking::whereIn('id', $this->selectedBookings)->where('status', 'Confirmed')->delete();
    ...
    {
        if ($value) {
            $this->selectedBookings = Booking::where('status', 'Confirmed')
                ->pluck('id')
                ->map(fn($id) => (int) $id)
                ->toArray();
        } else {
    ...
    {
        $allIds = Booking::where('status', 'Confirmed')->pluck('id')->map(fn($id) => (int) $id)->toArray();
    ...
    public function pendingBooking($bookingId)
    {
        $booking = Booking::findOrFail($bookingId);
        $booking->update(['status' => 'Pending']);

        session()->flash('success', 'Booking marked as pending successfully.');
    }
